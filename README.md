...
4. Date selection and the third/fourth NO activations show popups when their assets are available. Horse option taps show inline status; Horse Continue shows one experience meme per session: `horse-incident.png` for First Timer/Ridden Before or `horse-archers.png` for Basically a Professional. Both supplied horse images are installed. Missing or failed images still skip gracefully. Individual food choices use existing selection marks. Food Continue shows `caloric-surplus.png` once per page session, then advances on dismissal. The supplied asset is active; if it fails to load, the missing-image fallback advances directly. Reloading resets the session. The approval meme stays on the final result card. Missing popup images fall back to text.
